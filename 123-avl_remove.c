...
/**
 * __bst_search - searches for a value in a Binary Search Tree
 *
 * @tree: a pointer to the root node of the BST to search
 * @value: the value to search in the tree
 * Return: a pointer to the node containing a value equals to value.
 */
bst_t *__bst_search(const bst_t *tree, int value)
{
	...
	if (tree->n < value)
		return (__bst_search(tree->right, value));
	if (tree->n > value)
		return (__bst_search(tree->left, value));

	return ((bst_t *)tree);
}

/**
 * _bst_remove - removes a node from a Binary Search Tree
 *
